...
void dibuixarCanoAgent(float playerX,float playerZ);
...
void Agent::drawTank(){
	...
	glTranslatef(tx-(cell_size*XFactor),cell_size/10,tz-((int)cell_size*XFactor));
	...
	dibuixarCanoAgent(this->x/reductionFactor,this->z/reductionFactor);
...
void dibuixarCanoAgent(float playerX,float playerZ){
	...
	GLfloat mat_diffuse[] = {1.0, 0.0, 1.0, 1.0f};
	GLfloat mat_specular[] = {1.0, 0.0, 0.0, 1.0f};
	GLfloat vec[] = {1.0, 0.0,  0.0,  1.0};
	...
	//Light
	GLfloat lightvec[] = {1.0, 0.8,  0.5,  1.0};
	GLfloat light_position[] = {playerX-(float)cannonLength, 1.5,  playerZ-(float)cannonLength, 1.0f};
	glLightfv (GL_LIGHT2, GL_SPECULAR, lightvec);
	glLightfv (GL_LIGHT2, GL_DIFFUSE, lightvec);
